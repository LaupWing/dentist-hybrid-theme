...
<body <?php body_class(); ?>>
    ...
    <header class="absolute left-0 right-0 top-8 z-50 px-4">
        <div id="header-container" class="mx-auto flex max-w-7xl items-center justify-between rounded-full bg-white px-6 py-3 shadow-lg flex-col transition-[border-radius] duration-300 ease-in-out">
            ...
            <div id="mobile-menu" class="w-full overflow-hidden transition-all duration-300 ease-in-out md:hidden" style="max-height: 0;">
                <nav class="flex flex-col pt-4">
                    <a href="<?php echo esc_url(home_url('/')); ?>" class="border-t border-slate-100 py-3 text-sm font-bold text-slate-600 hover:text-indigo-700 <?php echo is_front_page() ? 'text-indigo-700' : ''; ?>">
                        HOME
                    </a>
                    <a href="<?php echo esc_url(home_url('/services/')); ?>" class="border-t border-slate-100 py-3 text-sm font-bold text-slate-600 hover:text-indigo-700 <?php echo (is_post_type_archive('service') || is_singular('service') || is_page_template('page-services.php') || is_page('services')) ? 'text-indigo-700' : ''; ?>">
                        SERVICES
                    </a>
                    <a href="<?php echo esc_url(home_url('/doctors/')); ?>" class="border-t border-slate-100 py-3 text-sm font-bold text-slate-600 hover:text-indigo-700 <?php echo (is_post_type_archive('doctor') || is_singular('doctor') || is_page_template('page-doctors.php') || is_page('doctors')) ? 'text-indigo-700' : ''; ?>">
                        DOCTORS
                    </a>
                    <a href="<?php echo esc_url(home_url('/blog/')); ?>" class="border-t border-slate-100 py-3 text-sm font-bold text-slate-600 hover:text-indigo-700 <?php echo (is_home() || is_singular('post') || is_category() || is_tag()) ? 'text-indigo-700' : ''; ?>">
                        BLOG
                    </a>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        toggle.addEventListener('click', function() {
            if (isOpen) {
                // Close menu - delay border-radius until menu is closed
                ...
                // Open menu - change border-radius immediately
